...
from app.models.status_update import StatusUpdate, StatusUpdateCreate, StatusUpdateUpdate
...
async def create_status_update(data: StatusUpdateCreate, session: Session = Depends(get_session)):
    return create_one(session, StatusUpdate, data.dict())

@router.patch("/{record_id}", response_model=StatusUpdate)
async def update_status_update(record_id: UUID, update_data: StatusUpdateUpdate, session: Session = Depends(get_session)):
    return update_one(session, StatusUpdate, record_id, update_data.dict(exclude_unset=True))

@router.delete("/{record_id}", response_model=dict)
async def delete_status_update(record_id: UUID, session: Session = Depends(get_session)):
